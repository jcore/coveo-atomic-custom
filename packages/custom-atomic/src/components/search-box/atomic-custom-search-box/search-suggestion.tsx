...
import { SearchBoxSuggestionElement } from '@coveo/atomic';
...
function getPartsForSuggestion(suggestion: SearchBoxSuggestionElement, isSelected: boolean) {
  let part = 'suggestion';
  if (isSelected) {
...
function getClassesForSuggestion(isSelected: boolean) {
  return `flex px-4 min-h-[40px] items-center text-left text-neutral-dark hover:bg-neutral-light cursor-pointer ${isSelected ? 'bg-neutral-light' : ''}`;
}
...
function ensureContentForRenderedSuggestion({ renderedSuggestion, suggestion }: { renderedSuggestion: HTMLElement; suggestion: SearchBoxSuggestionElement }) {
...
function getAriaLabelForRenderedSuggestion({
  ...
  const contentLabel = suggestion.ariaLabel ?? suggestion.query ?? renderedSuggestion.innerText ?? bindings.i18n.t('no-title');
  const labelWithType =
    isMacOS() && isButton
...
function getContentForSuggestion(suggestion: SearchBoxSuggestionElement) {
  return !isHTMLElement(suggestion.content) ? suggestion.content : <Fragment></Fragment>;
}

function getCommonSearchSuggestionAttributes(
  { bindings, id, suggestion, isSelected, side, index, lastIndex, isDoubleList }: SearchSuggestionProps,
  isButton: boolean,
...
export const SimpleSearchSuggestion: FunctionalComponent<SearchSuggestionProps> = props => {
  return <span {...getCommonSearchSuggestionAttributes(props, false)}>{getContentForSuggestion(props.suggestion)}</span>;
};

export const ButtonSearchSuggestion: FunctionalComponent<ButtonSearchSuggestionProps> = props => {
  return (
    <button
