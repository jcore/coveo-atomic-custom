...
export function AriaLiveRegion(regionName: string, assertive = false) {
  function getAriaLiveElement() {
    const event = buildCustomEvent<FindAriaLiveEventArgs>(findAriaLiveEventName, {});
  ...
  return (component: InitializableComponent<AnyBindings>, setterName: string) => {
  ...
  return (component: InitializableComponent<AnyBindings>, setterName: string) => {
    ...
    component.componentWillLoad = function () {
      ...
      this.componentDidRender = function () {
        ...
        if (focusAfterSearch && this.bindings.store.getUniqueIDFromEngine(this.bindings.engine) !== lastSearchId) {
      ...
      const focusTargetController: FocusTargetController = {
        ...
        focusAfterSearch: () => {
          lastSearchId = this.bindings.store.getUniqueIDFromEngine(this.bindings.engine);
        ...
        disableForCurrentSearch: () => this.bindings.store.getUniqueIDFromEngine(this.bindings.engine) !== lastSearchId && (focusAfterSearch = false),
      };
...
export function* getFocusableDescendants(element: Element): Generator<HTMLElement> {
