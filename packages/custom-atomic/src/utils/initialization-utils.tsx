import { ComponentInterface, getElement, h, forceUpdate as forceUpdateComponent } from '@stencil/core';
import { TOptions } from 'i18next';
import { buildCustomEvent } from './event-utils';
import { closest } from './utils';
import { AnyBindings } from '@coveo/atomic/dist/types/components/common/interface/bindings';
...
const initializableElements = ['atomic-recs-interface', 'atomic-search-interface', 'atomic-relevance-inspector', 'atomic-insight-interface', 'atomic-external'];
 ...
 */
export function initializeBindings<SpecificBindings extends AnyBindings>(element: Element) {
  return new Promise<SpecificBindings>((resolve, reject) => {
    const event = buildCustomEvent<InitializeEventHandler>(initializeEventName, bindings => resolve(bindings as SpecificBindings));
...
export function applyFocusVisiblePolyfill(element: HTMLElement) {
  ...
  window.addEventListener('focus-visible-polyfill-ready', () => window.applyFocusVisiblePolyfill?.(element.shadowRoot!), { once: true });
}
 ...
 */
export function InitializeBindings<SpecificBindings extends AnyBindings>({ forceUpdate }: InitializeBindingsProps = {}) {
  return (component: InitializableComponent<SpecificBindings>, bindingsProperty: string) => {
    ...
    component.componentWillLoad = function () {
      ...
      const event = buildCustomEvent(initializeEventName, (bindings: SpecificBindings) => {
        ...
        console.log('Init Bindings forceUpdate', forceUpdate);
        try {
    ...
    component.render = function () {
      if (this.error) {
        return <atomic-component-error element={getElement(this)} error={this.error}></atomic-component-error>;
      }

      console.log('Init Bindings render', this.bindings);
...
export function BindStateToController(
  ...
  console.log('BindStateToController ', controllerProperty);
  return (component: InitializableComponent<AnyBindings>, stateProperty: string) => {
    ...
    component.initialize = function () {
      ...
      console.log('BindStateToController ', this[controllerProperty]);

      if (!this[controllerProperty]) {
        return;
      }

      if (options?.onUpdateCallbackMethod && !this[options.onUpdateCallbackMethod]) {
      ...
      this.unsubscribeController = this[controllerProperty].subscribe(() => {
        ...
        options?.onUpdateCallbackMethod && this[options.onUpdateCallbackMethod]();
      });
    };
